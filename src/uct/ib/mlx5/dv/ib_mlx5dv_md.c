                                             ...
                                             uct_ib_md_t **p_md)
{
    ...
    if (uct_ib_mlx5_is_xgvmi_alias_supported(ctx)) {
        ...
        md->super.cap_flags |= UCT_MD_FLAG_SHARED_MKEY;
    ...
    } else if (UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, crossing_vhca_mkey)) {
        ucs_debug("%s: vhca_id=%u crossing_vhca_mkey is supported",
                  uct_ib_device_name(dev), md->super.vhca_id);
        md->super.vhca_id    = UCT_IB_MLX5DV_GET(cmd_hca_cap, cap, vhca_id);
        md->super.cap_flags |= UCT_MD_FLAG_SHARED_MKEY;
    } else {
        md->super.vhca_id = UINT32_MAX;
